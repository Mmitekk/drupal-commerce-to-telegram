...
1. Скачайте архив: **Code → Download ZIP** или файл
   `commerce_to_telegram-1.0.4.zip` со страницы
   [Releases](../../releases).
...
| `cURL error 28: Connection timed out` | Сервер хостинга не может соединиться с api.telegram.org. Модуль v1.0.4+ сам перебирает маршруты (ваш IP → официальные IP Telegram → DNS) и в тексте ошибки показывает все попытки — см. раздел ниже |
...
зависит — проблема на уровне сети хостинга.

**С версии 1.0.4 модуль сам перебирает маршруты** при каждой отправке:

1. IP из настройки «Прямое соединение по IP» (если указан);
2. официальные IP api.telegram.org — `149.154.167.198`, `149.154.167.220`;
3. обычный DNS-запрос.

В тексте ошибки тестовой отправки теперь видно **все попытки по очереди**
(какой маршрут и с какой ошибкой), а в журнале Drupal отмечается, каким
резервным маршрутом доставилось сообщение. Дальнейшие действия:

1. **Повторите тестовое сообщение** — если хоть один маршрут доступен,
   модуль доставит уведомление сам, без дополнительных настроек.
2. **Убедитесь, что SSH-проверка выполняется на том же сервере** (частая
   ошибка — проверка на другом своём сервере, где сеть другая). На
   сервере сайта команда `curl -sS --max-time 5 2ip.ru` должна показать
   его публичный IP. Затем сверьте доступность:
   ...
   Если второй вариант отвечает, а первый — нет, проблема в DNS хостинга:
   впишите рабочий IP в настройку «Прямое соединение по IP»
   (по умолчанию модуль и сам попробует оба официальных IP). Если рабочим
   оказался другой IP — укажите его; актуальный адрес можно узнать
   командой `nslookup api.telegram.org 8.8.8.8` с любой машины со
   здоровым DNS.
...
### v1.0.4
- **Автоперебор маршрутов до Telegram**: при каждой отправке модуль
  пробует по очереди IP из настройки «Прямое соединение по IP»,
  официальные IP api.telegram.org (`149.154.167.198`, `149.154.167.220`)
  и обычный DNS — пока какой-нибудь маршрут не сработает. Достаточно,
  чтобы был доступен хотя бы один адрес.
- **Подробная диагностика в ошибке тестовой отправки**: теперь видно все
  попытки по очереди (маршрут + текст ошибки соединения), а также
  подсказка, как проверить, что SSH-команды выполняются именно на
  сервере сайта (`curl -sS --max-time 5 2ip.ru`).
- Если Telegram ответил по сети (например, `Unauthorized` при неверном
  токене), перебор маршрутов не выполняется — это ошибка настроек, а не
  соединения.
